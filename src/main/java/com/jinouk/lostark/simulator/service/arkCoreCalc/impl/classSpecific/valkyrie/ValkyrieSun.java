...
public class ValkyrieSun  extends AbstractArkGrid {
    ...
    public ArkGridResponseDto getArkGrid(ArkGridRequestDto requestDto) {
        ...
        requestDto.getArkGridItems().forEach(item -> {
            ...
            } else if ("빛의 은총".equals(name)) {
                calcGraceOfLight(effectsMap, point, grade);
            } else if ("성스러운 맹약".equals(name)) {
                calcSacredCovenant(effectsMap, point, grade);
            } else if ("빛이 생명을 새긴".equals(name)) {
                calcLifeEngravedByLight(effectsMap, point, grade);
            }
    ...
    //
    private void calcGraceOfLight(Map<String, Double> effectsMap, int point, String grade) {
        if("유물".equals(grade)) {
            switch (point){
    ...
    //
    private void calcSacredCovenant(Map<String, Double> effectsMap, int point, String grade) {
        if("유물".equals(grade)) {
            switch (point){
                case 10: mergeEffect(effectsMap, "아피강", 1.50); break;
    ...
    //
    private void calcLifeEngravedByLight(Map<String, Double> effectsMap, int point, String grade) {
        if("유물".equals(grade)) {
            switch (point){
                case 10: mergeEffect(effectsMap, "아피강", 1.50); break;
                case 14: mergeEffect(effectsMap, "아피강", 1.50); break;
