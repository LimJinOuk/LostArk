...
public class HolyKnightSun extends AbstractArkGrid {
    ...
    public ArkGridResponseDto getArkGrid(ArkGridRequestDto requestDto) {
        ...
        requestDto.getArkGridItems().forEach(item -> {
            ...
            } else if ("천상의 대리인".equals(name)) {
                calcCelestialAgent(effectsMap, point, grade);
            } else if ("진정한 정의".equals(name)) {
                calcTrueJustice(effectsMap, point, grade);
            } else if ("빛의 축복".equals(name)) {
                calcBlessingOfLight(effectsMap, point, grade);
            }
    ...
    //
    private void calcCelestialAgent(Map<String, Double> effectsMap, int point, String grade) {
        if("유물".equals(grade)) {
            switch (point){
    ...
    //
    private void calcTrueJustice(Map<String, Double> effectsMap, int point, String grade) {
        if("유물".equals(grade)) {
            switch (point){
                case 10: mergeEffect(effectsMap, "아피강", 1.50); break;
    ...
    //
    private void calcBlessingOfLight(Map<String, Double> effectsMap, int point, String grade) {
        if("유물".equals(grade)) {
            switch (point){
                case 10: mergeEffect(effectsMap, "아피강", 1.50); break;
                case 14: mergeEffect(effectsMap, "아피강", 1.50); break;
