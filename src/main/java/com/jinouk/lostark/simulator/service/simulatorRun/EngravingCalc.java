...
public class EngravingCalc {
    ...
    static {
        handlers.put("결투의 대가", (v, r) -> { r.put("적주피", v.get(0)); r.put("헤드어택시 추가 피증", v.get(1));});
        handlers.put("기습의 대가", (v, r) ->{ r.put("적주피", v.get(0)); r.put("백어택시 추가 피증", v.get(1));});
        handlers.put("마나 효율 증가", (v, r) -> { r.put("적주피", v.get(0));});
        handlers.put("바리케이드", (v, r) -> { r.put("적주피", v.get(0));});
        ...
        handlers.put("약자 무시", (v, r) -> { r.put("적주피", v.get(0));});
        handlers.put("예리한 둔기", (v, r) -> { r.put("치피증", v.get(0));});
        handlers.put("돌격대장", (v, r) -> { r.put("이속증가분의 n퍼만큼 피증", v.get(0));});
        handlers.put("원한", (v, r) -> { r.put("적주피", v.get(0));});
        handlers.put("저주받은 인형", (v, r) -> { r.put("적주피", v.get(0));});
